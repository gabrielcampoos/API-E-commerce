...
exports.validarParamentrosFiltragemProdutos = void 0;
function validarParamentrosFiltragemProdutos(request, response, next) {
    const filtros = request.query;
    if (filtros.ordem_nome !== "cresc" && filtros.ordem_nome !== "decresc") {
        return response.status(400).json({
            status: false,
            mensagem: "Os valores dos filtros para ordenação do nome do produto é inválido.",
        });
    }
    if (filtros.ordem_preco !== "cresc" && filtros.ordem_preco !== "decresc") {
        return response.status(400).json({
            status: false,
            mensagem: "Os valores dos filtros para ordenação do preço do produto é inválido.",
        });
    }
...
exports.validarParamentrosFiltragemProdutos = validarParamentrosFiltragemProdutos;
